...
// usage: NODE_PATH=<global node_modules with playwright> node render_report.cjs <filePath(확장자 없이)> '<paramsJson>' <outPng> [ratio]
...
(async () => {
  const [filePath, paramsJson, outPng, ratio] = process.argv.slice(2);
